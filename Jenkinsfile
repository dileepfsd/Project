jenkinsPipeLine {
     ...
     stage('Step 1 : Git Checkout') {
       git credentialsId: 'Dtp@1611', url: 'https://github.com/dileepfsd/Project.git'
       echo 'Done : Git checkout'
     }

   stage('Step 2 : Build Image') {
         sh 'docker build -t thattan79/project-":$BUILD_NUMBER"'
    }
   ...
   stage('Step 3 : Deliver Image') {
   		...
   		sh 'docker push thattan79/project-":$BUILD_NUMBER"'
   }
   ...
   //MySql Standalone
       ...
       // Spring boot application
         sh 'docker run -p 8080:8080 --name project-":$BUILD_NUMBER"--link mysql-standalone:mysql -d project-":$BUILD_NUMBER"'
      }
}
